...
class BluetoothManager:
    ...
    def handle_new_connection(self, device_path: str):
        """Gère une nouvelle connexion avec configuration A2DP"""
        ...
        if not self.active_device:
            ...
            self._setup_a2dp_profile(device_path)
        else:
    ...
    def _get_media_transport_path(self, device_path: str) -> Optional[str]:
        """Récupère le chemin du MediaTransport actif pour un appareil"""
        try:
            objects = self.obj_manager.GetManagedObjects()
            device_path_prefix = device_path.replace("/org/bluez/", "")
            
            for path, interfaces in objects.items():
                if "org.bluez.MediaTransport1" not in interfaces:
                    continue
                    
                if device_path_prefix in path:
                    props = interfaces["org.bluez.MediaTransport1"]
                    if props.get("State", "") in ["active", "pending"]:
                        return path
            
            return None
        except Exception as e:
            print(f"Erreur lors de la recherche du MediaTransport: {e}")
            return None

    def _setup_a2dp_profile(self, device_path: str) -> bool:
        """Configure le profil A2DP pour un appareil"""
        try:
            device = self.bus.get_object('org.bluez', device_path)
            ...
            if a2dp_sink_uuid not in str(uuids):
                print(f"L'appareil {device_path} ne supporte pas A2DP")
                return False
            
            # Définir l'appareil comme trusted
            device_props.Set('org.bluez.Device1', 'Trusted', True)
            
            # Attendre que le MediaTransport soit disponible
            for _ in range(5):  # 5 tentatives
                transport_path = self._get_media_transport_path(device_path)
                if transport_path:
                    try:
                        transport = self.bus.get_object('org.bluez', transport_path)
                        transport_iface = dbus.Interface(transport, 'org.bluez.MediaTransport1')
                        transport_iface.Acquire()
                        print(f"Profil A2DP configuré avec succès pour {device_path}")
                        return True
                    except Exception as e:
                        print(f"Erreur lors de l'activation du transport A2DP: {e}")
                
                print("Attente du MediaTransport...")
                asyncio.sleep(1)
            
            print("Impossible de trouver le MediaTransport après plusieurs tentatives")
            return False
                    
        except Exception as e:
            print(f"Erreur lors de la configuration A2DP: {e}")
            return False

    def _transfer_a2dp(self, old_device_path: str, new_device_path: str) -> bool:
        """Transfère le profil A2DP d'un appareil à un autre"""
        try:
            # Désactiver A2DP sur l'ancien appareil
            if old_device_path:
                try:
                    transport_path = self._get_media_transport_path(old_device_path)
                    if transport_path:
                        transport = self.bus.get_object('org.bluez', transport_path)
                        transport_iface = dbus.Interface(transport, 'org.bluez.MediaTransport1')
                        transport_iface.Release()
                        print(f"A2DP désactivé sur l'ancien appareil")
                except Exception as e:
                    print(f"Erreur lors de la désactivation de l'ancien A2DP: {e}")

            # Attendre un court instant pour laisser le temps au système de gérer la transition
            asyncio.sleep(0.5)

            # Configurer A2DP sur le nouvel appareil
            if new_device_path:
                success = self._setup_a2dp_profile(new_device_path)
                if not success:
                    print("Échec de la configuration A2DP sur le nouvel appareil")
                    return False
                        
            return True
        except Exception as e:
            print(f"Erreur lors du transfert A2DP: {e}")
            return False

    async def switch_devices(self, old_address: str, new_address: str):
        """Gère le changement d'appareil actif avec transfert A2DP"""
        ...
        if (self.active_device and self.active_device['address'] == old_address and 
            ...
            new_path = f"/org/bluez/hci0/dev_{'_'.join(new_address.split(':'))}"
            
            # Mettre à jour les états des appareils avant le transfert A2DP
            temp_active = self.active_device
            self.active_device = self.pending_device
            self.pending_device = None
            
            # Transférer A2DP
            if await self._transfer_a2dp_async(old_path, new_path):
                # Déconnecter l'ancien appareil
                self.disconnect_device(old_path)
            else:
                # Restaurer les états en cas d'échec
                print("Échec du transfert A2DP - restauration des états")
                self.active_device = temp_active
                self.pending_device = self.active_device
                self._check_existing_connections()
    ...
    async def _transfer_a2dp_async(self, old_path: str, new_path: str) -> bool:
        """Version asynchrone du transfert A2DP"""
        def transfer():
            return self._transfer_a2dp(old_path, new_path)
        
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, transfer)
